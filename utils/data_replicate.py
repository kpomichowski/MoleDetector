...
def replicate_rows_by_k_factor(df: pd.DataFrame, k_factors: list) -> pd.DataFrame:
    ...
    assert len(df.dx.unique()) == len(k_factors), 'inaproperiate length of k, must equals to number of columns in df'
    df_copy = df.copy(deep=True)
    for k_index in range(len(k_factors)):
        df_copy = df_copy.append([df_copy.loc[df_copy['lesion_type'] == k_index,:]] * (k_factors[k_index]), ignore_index=True)
