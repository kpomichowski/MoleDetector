...
class FocalLoss(torch.nn.Module):
    ...
    def __init__(self, gamma=2, alpha=None, reduction='mean'):
        ...
        if reduction not in ('mean', 'sum', 'none'):
            raise RuntimeError(f'Inaproperiate reduction value. Available reductions: `mean`, `sum`, `none`.')
    ...
    def forward(self, input: torch.tensor, target: torch.tensor) -> torch.tensor:
        ...
        if is_alpha:
            assert self.alpha.size() == pt.size() == CE_loss.size(), "Batch size not equal to the num of weights in alpha."
...
def focal_loss(device: str, gamma: int = 2, reduction: str = 'mean', alpha: torch.Tensor or None = None) -> FocalLoss:

    if alpha is not None:
        alpha = alpha.to(device) if torch.is_tensor(alpha) else torch.tensor(alpha).to(device)
